...
async function preload() {
  ...
  Bl=loadImage("MAIN/blood.png")
  HImg=loadImage("MAIN/H.png")
  EnemM=loadImage("MAIN/EnemyMisssi.png")
  MissileTruckImg=loadImage("MAIN/RT.png")
  MTImg=loadImage("MAIN/Tank.png")
}
...
async function draw() {
  ...
  if (gameState === PoliceMode) {
    ...
    if(AmmoGr.isTouching(PoliceGr)){
      ...
      Ex1Gr.add(Ex1)
      CarEX.play();
      bloodC=createSprite(policeC.x,policeC.y,10,10)
      bloodC.scale=0.7
      bloodC.addImage(Bl)
      BloodGr.add(bloodC)
      PoliceKilled+=1
    ...
    if(Ammo1Gr.isTouching(PoliceGr)){
      ...
      if(ammo===0||ammo<0){
        ...
        CarEX.play();
        PoliceKilled+=1
        bloodC1=createSprite(policeC.x,policeC.y,10,10)
        bloodC1.scale=0.7
        bloodC1.addImage(Bl)
        Blood1Gr.add(bloodC1)
        Ammo1Gr.destroyEach();
      policeC.destroy();
      ammo=5
      }
    ...
    if(PoliceKilled===15||PoliceKilled>15){
     ...
     if(AmmoGr.isTouching(MTGr)){
      ...
      CarEX.play();
      PoliceKilled+=1
      bloodC2=createSprite(MiltaryTank.x,MiltaryTank.y,10,10)
      bloodC2.scale=0.8
      bloodC2.addImage(Bl)
      Blood2Gr.add(bloodC2)
       MTGr.destroyEach();
       CarEX.play();
     }
     else{
       Blood2Gr.destroyEach();
       Ex2Gr.destroyEach();
     }
     if(AmmoGr.isTouching(MTruckGr)){
      ...
      CarEX.play();
      PoliceKilled+=1
      bloodC3=createSprite(Miltarytruck.x,Miltarytruck.y,10,10)
      bloodC3.scale=0.8
      bloodC3.addImage(Bl)
      Blood3Gr.add(bloodC3) 
      MTruckGr.destroyEach();

       CarEX.play();
     }
     else{
      Blood3Gr.destroyEach();
      Ex3Gr.destroyEach();
     }
...
if(gameState!==FIRSTBG&&gameState!==START&&gameState!==SELECTOR&&gameState!==CARSELECTOR&&gameState!==SETTINGS&&gameState!==INS){
  ...
  text("Police Dead:"+PoliceKilled,200,100)
}
